...
//	private final Context mContext;
	...
	SingleBTChannelManagerServer(Handler handler, Executor executor, Context context) {
//		mContext = context;
		...
		HandlerThread thread = new HandlerThread("SCMS");
		thread.start();
...
//				mChannelMap.clear();
		...
		public void handleMessage(Message msg) {
			...
			case NEW_CLIENT_MSG: {
				...
				if (env.getUUID(BluetoothChannel.CUSTOM, false).equals(server.getUUID())) {
					notifyStateChanged(TransportStateMachine.S_CONNECTED);
				} else {
					logw("unknow main channel:" + server.getUUID() + " with NEW_CLIENT_MSG");
				}
				break;
			}
			...
			case CLIENT_CLOSE_MSG: {
				...
				if (!env.isMainChannel(channel.getUUID(), false)) {
					logv("not main channel ingore it");
				}
				
				if (env.getUUID(BluetoothChannel.CUSTOM, false).equals(uuid)) {
					clearChannel();
				}
				break;
			}
